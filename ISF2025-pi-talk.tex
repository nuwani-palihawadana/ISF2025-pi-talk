...
\phantomsection\label{nonparametric-model}
\placefig{0.5}{1.4}{width=15cm}{nonpara1}
\end{frame}

\begin{frame}{Nonparametric model}
\phantomsection\label{nonparametric-model-1}
...
\begin{frame}{BB with time series cross-validation}
\phantomsection\label{bb-with-time-series-cross-validation}
...
\begin{frame}{BB with time series cross-validation}
\phantomsection\label{bb-with-time-series-cross-validation-1}
\begin{block}{}
\fontsize{9}{9}\sf
...
\begin{frame}{CP with time series cross-validation (Wang \& Hyndman
2024)}
\phantomsection\label{cp-with-time-series-cross-validation-wang-hyndman-2024}
...
\begin{frame}{CP with time series cross-validation (Wang \& Hyndman
2024)}
\phantomsection\label{cp-with-time-series-cross-validation-wang-hyndman-2024-1}
\begin{block}{}
\fontsize{9}{9}\sf
...
\begin{itemize}
  ...
  \begin{itemize}
  \item \color{violet} Preserves temporal dependencies  \newline
  \item \color{violet} Accounts for additional uncertainty brought into the process by ex-ante forecasting
\end{itemize}
\end{itemize}
\end{frame}

\begin{frame}{CB with time series cross-validation}
\phantomsection\label{cb-with-time-series-cross-validation}
\placefig{1}{2.5}{width=10cm}{cpsplit}
...
\begin{frame}{CB with time series cross-validation}
\phantomsection\label{cb-with-time-series-cross-validation-1}
\begin{block}{}
\fontsize{9}{9}\sf
\color{blue} \textbf{Step 1 :} \color{black} Split data into an initial training window, $\{\bm{z}_{1}, \dots, \bm{z}_{tr}\}$, an initial calibration window, $\{\bm{z}_{tr + 1}, \dots, \bm{z}_{tr + tc}\}$, and an initial test window, $\{\bm{z}_{tr + tc + 1}, \dots, \bm{z}_{tr + tc + H}\}$.
\end{block}
...
\begin{frame}{Open-source implementation}
\phantomsection\label{open-source-implementation}
...
\begin{itemize}
    \item Barber, RF, EJ Candès, A Ramdas \& RJ Tibshirani (2023). Conformal Prediction Beyond Exchangeability. \textit{The Annals of Statistics} 51(2), 816–845.
    \item Gibbs, I \& E Candès (2021). Adaptive Conformal Inference Under Distribution Shift. \textit{Advances in Neural Information Processing Systems}, 1660–1672.
    \item Vovk, V, A Gammerman \& G Shafer (2005). \textit{Algorithmic Learning in a Random World}. New York, NY: Springer.
    \item Wang, X \& RJ Hyndman (2024). Online Conformal Inference for Multi-Step Time Series Forecasting. \textit{arXiv preprint arXiv:2410.13115}.
\end{itemize}
\end{frame}
